...
function Utils.GiveVehicleKey(plate, entity)
    if shared.framework ~= 'esx' then -- Qb or Qbox
        if not plate and entity and DoesEntityExist(entity) then
...
function Utils.GetVehicleDoorPosition(vehicle)
    local model = GetEntityModel(vehicle)
    local isModel1 = model == joaat(Config.TaskVehicles.model_1)
    if not isModel1 then
        return GetOffsetFromEntityInWorldCoords(vehicle, 0.0, -5.0, 0.0)
    end
    local boneName = isModel1 and 'bonnet' or 'boot'
    local worldPos = GetWorldPositionOfEntityBone(vehicle, GetEntityBoneIndexByName(vehicle, boneName))
