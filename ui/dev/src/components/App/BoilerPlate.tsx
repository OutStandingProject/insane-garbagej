...
const BoilerPlate: React.FC<BoilerPlateProps> = ({ children }) => {
  ...
  const { setIsAppRunning } = useData();

  return (
    <div
      className={classNames(
        "w-full h-full flex flex-col transition-[padding]",
        {
          "p-8": !isAppFullScreen,
        }
      )}
    >
      ...
      <div
        className={classNames(
          "bg-black/90 bg-cover bg-no-repeat bg-center transition-[border-radius] w-full h-full overflow-auto scrollbar-hide",
          {
