...
-- NUI: iniciar tarefa
-- Garante que o jogador tem uma lobby antes de iniciar a task.
-- Se nao tiver lobby, cria uma solo no servidor e so depois inicia.
RegisterNUICallback('nui:startLobbyWithTask', function(data, cb)
    -- Aceita tanto numero direto como tabela { taskId = X }
    local taskId
    ...
    -- Fecha o NUI antes para libertar o controlo do jogador
    SetNuiFocus(false, false)
    cb({})

    -- Se ainda nao tem lobby, cria uma solo agora
    if not client.lobby or not client.lobby.id then
        ...
        if lobbyResult and lobbyResult.lobbyId then
            ...
            Utils.Notify('Erro ao criar lobby. Tenta de novo.', 'error')
            return
        end
    end
...
-- Resposta do servidor ao convite
...
-- SERVER NET EVENT: Convidar jogador para a lobby pelo ID
RegisterNetEvent('insane-garbagej:server:invitePlayer', function(targetId)
    local src = source
    if not targetId or not GetPlayerName(targetId) then
        TriggerClientEvent('insane-garbagej:client:inviteResponse', src, { success = false, message = 'Jogador nao encontrado.' })
        return
    end
    TriggerClientEvent('insane-garbagej:receiveLobbyInvite', targetId, src)
    TriggerClientEvent('insane-garbagej:client:inviteResponse', src, { success = true })
end)
